...
import { RESPONSE_API, timeDelay } from '../../services/common';
...
export const ProductForm = ( props ) =>
{
	...
	useEffect( () =>
	{
		if ( product )
		{
			...
			file.push({
				...
				name: product.avatar,
				status: 'done',
				url: product.avatar || DEFAULT_IMG,
				default: true
			});
	...
	{
		const response = await submitFormProduct( id, files, e, dispatch, history );
		await RESPONSE_API(response, message, id, history, 'product');
	}
				...
				>
					<div className='mb-3'>
						...
						<div className='row'>
							...
							<div className='col-md-3'>
								<Form.Item name="sale" label="Product sale"
									className='d-block'>
									<Input className='form-control' placeholder='Enter sale' />
								</Form.Item>
							</div>
							<div className='col-md-3'>
